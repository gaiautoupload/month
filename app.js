...
function renderHistory() {
  ...
  const liveTrades = current.signalMonth && !current.closed ? currentPositions.map((row) => ({
    month: current.signalMonth,
    stockId: row.stockId,
    name: row.name,
    returnPct: Number(row.returnPct || 0),
    hit20: Number(row.returnPct || 0) >= 20,
    ongoing: true,
  })) : [];
  const tradeRows = [...liveTrades, ...[...data.historicalTrades].reverse()];
  document.getElementById("tradeRows").innerHTML = tradeRows.map((row) => `<tr class="${row.ongoing ? "ongoing-row" : ""}"><td>${row.month}</td><td><span class="month-status ${row.ongoing ? "live" : ""}">${row.ongoing ? "進行中" : "已結算"}</span></td><td>${escapeHtml(row.stockId)}</td><td>${escapeHtml(row.name)}</td><td class="${cls(row.returnPct)}">${fmtPct(row.returnPct)}</td><td>${row.hit20 ? "是" : "否"}</td></tr>`).join("");
}
